...
public class DatabaseHelperDrugs extends SQLiteOpenHelper {
    ...
    private String CREATE_DRUG_TABLE =
    ...
    private String DROP_DRUG_TABLE = "DROP TABLE IF EXISTS " + TABLE_DRUG;
    ...
    public void addDrug(Drugs drugs){
        ...
        values.put(COLUMN_DRUG_NAME, drugs.getNume());
        values.put(COLUMN_DRUG_CODE, drugs.getCod());
        values.put(COLUMN_DRUG_DESCRIPTION, drugs.getDescriere());
        values.put(COLUMN_DRUG_STOCK, drugs.getStoc());
    ...
    public Cursor getDrugs(){
        ...
        Cursor cursor = db.rawQuery("SELECT * FROM drug ORDER BY drug_name", null);
        return cursor;
    }
    ...
    public boolean checkDrugExists(int id){
        ...
        Cursor cursor = db.query(TABLE_DRUG,    //Tabel pt query
                columns,
                selection,                      //where
    ...
    public int getDrugPrice(int id){
        ...
        if(cursor.moveToFirst() && cursor.getCount() >= 1){
            final int pretIndex = cursor.getColumnIndex(COLUMN_DRUG_PRICE);
            pret = cursor.getInt(pretIndex);
            return pret;
        }
        return -1;
    }
    ...
    public String getDrugName(int id){
        ...
        String nume = "";
        if(cursor.moveToFirst() && cursor.getCount() >= 1){
            final int numeIndex = cursor.getColumnIndex(COLUMN_DRUG_NAME);
            nume = cursor.getString(numeIndex);
        }
        return nume;
    }
